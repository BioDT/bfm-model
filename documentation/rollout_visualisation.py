...
def plot_raw_triple(var_name: str,
                    pred_tensor: torch.Tensor,
                    gt_tensor: torch.Tensor,
                    timestamps: list) -> go.Figure:
    """
    ...
    fig = make_subplots(
        rows=1, cols=3,
        subplot_titles=(
            f"Ground Truth @ {t_label}",
            f"Prediction @ {t_label}",
            "Difference"
        )
    )
    # Add traces
    ...
    fig.update_layout(
        title_text=f"{var_name} Comparison",
        showlegend=False,
...
with tab1:
    ...
    animation_type = st.sidebar.radio(
        "Choose timeline to animate", ["Prediction", "Ground Truth"]
    )
    ...
    for rec in records:
        ...
        frame_fig = px.imshow(
            da, origin="lower", aspect="equal", color_continuous_scale="Viridis"
        )
        frame_fig.update_layout(title=str(rec[key].batch_metadata.timestamp[-1] if isinstance(rec[key].batch_metadata.timestamp,(list,tuple)) else rec[key].batch_metadata.timestamp), margin=dict(l=0,r=0,t=30,b=0))
...
with tab2:
    ...
    fig_pre = plot_raw_triple(var_pre,
                              pd_pre[grp_pre][var_pre],
                              gd_pre[grp_pre][var_pre],
                              tns_pre)
    ...
    st.download_button(
        f"Download {var_pre}",
        data=png_pre,
        file_name=f"pre_{grp_pre}_{var_pre}_step{pre_step}.png",
        mime='image/png'
    )
