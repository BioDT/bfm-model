...
class BFMDecoder(nn.Module):
    def __init__(
        ...
        drop_rate: float = 0.1,
        depth: int = 2,
        mlp_ratio: float = 4.0,  # unused currently, could be passed to perceiver IO (requires adaptation)
        perceiver_ln_eps: float = 1e-5,  # unused currently, could be passed to perceiver IO (requires adaptation)
    ...
    def forward(self, x, batch, lead_time):
        ...
        absolute_times = torch.tensor([t.timestamp() / 3600 for t in batch.metadata.time], dtype=torch.float32, device=x.device)
        absolute_time_encode = self._time_encoding(absolute_times)
        absolute_time_embed = self.absolute_time_embed(absolute_time_encode)
        queries = queries + absolute_time_embed.unsqueeze(1)
